...
def extract_doc_id(query):
    match = re.search(r"document\s*(\d+)", query.lower())
...
def generate(query, chunks):
    ...
    prompt = f"""
You are a strict, document-aware QA assistant.

Rules:
1. The context is divided into sections labeled "### Document X"
2. Each section belongs to a different document
3. If the question refers to a specific document:
   → ONLY use that document's content
4. Do NOT mix information across documents unless explicitly required
5. If the answer is directly present → extract it
6. If the answer requires combining multiple chunks → you MAY connect ideas logically
7. Do NOT use any outside knowledge

Special Cases:
8. If the answer is NOT present in the context:
   → respond exactly: "I don't know"
9. If the question asks to invent, assume, or create something not grounded in context:
   → respond exactly: "I cannot generate that from the given context"
